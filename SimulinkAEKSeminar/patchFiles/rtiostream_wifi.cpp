...
{
	/* If scheduler interrupts are not disabled then exit 
	 * routines for blocks will not be retained in external mode.
	 * This is because when interrupts are still active there is 
	 * a possibility that after the exit routine step function will
	 * be executed again.
	 */
	disable_rt_OneStep();
